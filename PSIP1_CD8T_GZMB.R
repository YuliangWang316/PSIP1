...
CD8T<-subset(HNSCC,idents = c("7","10","15"))
...
for (i in 1:26) {
  ...
  #colnames(e)<-"A"
  assign(paste("scaldata_",i,"_TIL",sep = ""),data.frame(mean(c$PSIP1),mean(f$GZMB)))
...
rm(f)
colnames(scaldata)<-c("PSIP1","GZMB")
library(ggplot2)
library(ggpubr)
ggplot(data = scaldata,aes(x=PSIP1,y=GZMB)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=TRUE,size=1.5,color="red")+stat_cor(data = scaldata,method = "pearson") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 
