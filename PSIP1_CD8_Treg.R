...
colnames(scaldata)<-c("PSIP1","CD8T_Treg")
library(ggplot2)
library(ggpubr)
ggplot(data = scaldata,aes(x=PSIP1,y=CD8T_Treg)) + geom_point(color="blue",size=3.4) + stat_smooth(method = "lm",se=TRUE,size=1.5,color="red")+stat_cor(data = scaldata,method = "spearman") +  theme_set(theme_bw()) +  theme_set(theme_bw()) 
